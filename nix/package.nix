...
let
  ...
  srcMap = {
    x86_64-linux = fetchurl {
      url = "https://github.com/larteragia/micah/releases/download/v${version}/Micah_${version}_amd64.deb";
      hash = sources.hashes.x86_64-linux;
    };
    x86_64-darwin = fetchurl {
      url = "https://github.com/larteragia/micah/releases/download/v${version}/Micah_x64.app.tar.gz";
      hash = sources.hashes.x86_64-darwin;
    };
    aarch64-darwin = fetchurl {
      url = "https://github.com/larteragia/micah/releases/download/v${version}/Micah_aarch64.app.tar.gz";
...
assert lib.assertMsg (builtins.hasAttr sys srcMap)
  "micah: unsupported platform ${sys}";
...
stdenv.mkDerivation {
  ...
  installPhase = if stdenv.hostPlatform.isLinux then ''
    ...
    install -Dm755 usr/bin/micah $out/bin/micah

    wrapProgram $out/bin/micah \
      "''${gappsWrapperArgs[@]}" \
  ...
  meta = with lib; {
    ...
    homepage = "https://github.com/larteragia/micah";
