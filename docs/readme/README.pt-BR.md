<div align="center">
  ...
  <p><a href="https://github.com/larteragia/micah">Site</a> · <a href="https://github.com/larteragia/micah/tree/main/docs">Documentação</a> · <a href="https://github.com/larteragia/micah">Código-fonte do site</a></p>

  <p>
    <img src="https://img.shields.io/github/v/release/larteragia/micah?label=version&color=blue" alt="versão" />
...
- **Fluxo com agentes:** planos, subagentes, memória do projeto via `MICAH.md`, leitura / escrita / edição / multiedição / grep / glob, bash com aprovação e processos em segundo plano
...
Os instaladores mais recentes estão na página de [Releases](https://github.com/larteragia/micah/releases/latest). O Micah se atualiza automaticamente por ela.
...
- **Arch / AUR:** `yay -S micah-bin` ou `paru`. Acompanha a versão mais recente.
- **NixOS / Nix:** use o flake oficial com `nix profile install github:larteragia/micah` fora do NixOS. No NixOS, importe o flake e adicione `inputs.micah.packages.${pkgs.system}.micah` a `environment.systemPackages`. `nixosModules.micah` também oferece uma configuração simplificada.
- **AppImage:** requer FUSE. Sem ele: `./Micah_*.AppImage --appimage-extract-and-run`. Em caso de falhas no Wayland, tente `WEBKIT_DISABLE_DMABUF_RENDERER=1`. Os pacotes `.deb` / `.rpm` usam a pilha GTK do sistema e costumam ser mais suaves.
...
Micah é licenciado sob a Apache-2.0. Para informações sobre dependências, consulte a [Apache License 2.0](../../LICENSE).
