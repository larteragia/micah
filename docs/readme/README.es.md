<div align="center">
  ...
  <p>
    <a href="https://github.com/larteragia/micah">Sitio web</a> ·
    <a href="https://github.com/larteragia/micah/tree/main/docs">Documentación</a> ·
    <a href="https://github.com/larteragia/micah">Código fuente del sitio web</a>
  </p>

  <p>
    <img src="https://img.shields.io/github/v/release/larteragia/micah?label=version&color=blue" alt="versión" />
    <img src="https://img.shields.io/github/downloads/larteragia/micah/total?label=downloads&color=blue" alt="descargas" />
...
- Adjunta archivos y selecciones directamente al panel lateral de IA
...
- **Flujo con agentes:** planes, subagentes, memoria de proyecto mediante `MICAH.md`, lectura / escritura / edición / edición múltiple / grep / glob, bash con aprobación y procesos en segundo plano
...
Los instaladores más recientes están en la página de [Releases](https://github.com/larteragia/micah/releases/latest). Micah se actualiza automáticamente desde allí.
...
- **Arch / AUR:** `yay -S micah-bin` (o `paru`, etc.). Sigue la versión más reciente.
- **NixOS / Nix:** usa el flake oficial: `nix profile install github:larteragia/micah` en sistemas que no sean NixOS, o importa el flake y añade `inputs.micah.packages.${pkgs.system}.micah` a `environment.systemPackages` en NixOS. También está disponible `nixosModules.micah` para una configuración más sencilla.
- **AppImage:** requiere FUSE. Sin FUSE: `./Micah_*.AppImage --appimage-extract-and-run`. Si hay problemas de renderizado en Wayland, prueba `WEBKIT_DISABLE_DMABUF_RENDERER=1`. Los paquetes `.deb` / `.rpm` enlazan con la pila GTK del sistema y suelen funcionar con mayor fluidez.

## Configurar la IA

1. Abre **Ajustes -> IA**.
2. Elige un proveedor y pega tu clave API. Para inferencia local, apunta Micah a tu endpoint de LM Studio / MLX / Ollama.
...
Micah se distribuye bajo la licencia Apache-2.0. Para más información sobre las dependencias, consulta [Apache License 2.0](../../LICENSE).
