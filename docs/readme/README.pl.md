<div align="center">
  ...
  <p><a href="https://github.com/larteragia/micah">Strona</a> · <a href="https://github.com/larteragia/micah/tree/main/docs">Dokumentacja</a> · <a href="https://github.com/larteragia/micah">Kod źródłowy strony</a></p>

  <p>
    <img src="https://img.shields.io/github/v/release/larteragia/micah?label=version&color=blue" alt="wersja" />
    <img src="https://img.shields.io/github/downloads/larteragia/micah/total?label=downloads&color=blue" alt="pobrania" />
...
- **Agentowy przepływ pracy:** plany, podagenci, pamięć projektu przez `MICAH.md`, odczyt / zapis / edycja / wielokrotna edycja / grep / glob, bash wymagający zgody i procesy w tle
...
Najnowsze instalatory znajdują się na stronie [Releases](https://github.com/larteragia/micah/releases/latest). Micah aktualizuje się stamtąd automatycznie.
...
- **Arch / AUR:** `yay -S micah-bin` lub `paru`. Pakiet śledzi najnowsze wydanie.
- **NixOS / Nix:** użyj oficjalnego flake. Poza NixOS uruchom `nix profile install github:larteragia/micah`. W NixOS zaimportuj flake i dodaj `inputs.micah.packages.${pkgs.system}.micah` do `environment.systemPackages`. Dostępny jest też prostszy moduł `nixosModules.micah`.
- **AppImage:** wymaga FUSE. Bez niego uruchom `./Micah_*.AppImage --appimage-extract-and-run`. Przy błędach renderowania w Wayland spróbuj `WEBKIT_DISABLE_DMABUF_RENDERER=1`. Pakiety `.deb` / `.rpm` korzystają z systemowego GTK i zwykle działają płynniej.
...
Micah jest objęty licencją Apache-2.0. Informacje o zależnościach znajdziesz w [Apache License 2.0](../../LICENSE).
