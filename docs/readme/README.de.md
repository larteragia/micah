<div align="center">
  ...
  <p>
    <a href="https://github.com/larteragia/micah">Website</a> ·
    <a href="https://github.com/larteragia/micah/tree/main/docs">Dokumentation</a> ·
    <a href="https://github.com/larteragia/micah">Quellcode der Website</a>
  </p>

  <p>
    <img src="https://img.shields.io/github/v/release/larteragia/micah?label=version&color=blue" alt="Version" />
    <img src="https://img.shields.io/github/downloads/larteragia/micah/total?label=downloads&color=blue" alt="Downloads" />
...
- **Agentischer Workflow:** Pläne, Sub-Agenten, Projektgedächtnis über `MICAH.md`, Lesen / Schreiben / Bearbeiten / Mehrfachbearbeitung / grep / glob, Bash mit Freigabe und Hintergrundprozesse
...
Die neuesten Installationspakete stehen auf der Seite [Releases](https://github.com/larteragia/micah/releases/latest). Micah aktualisiert sich von dort automatisch.
...
- **Arch / AUR:** `yay -S micah-bin` (oder `paru` usw.). Folgt der neuesten Version.
- **NixOS / Nix:** Nutze den offiziellen Flake: `nix profile install github:larteragia/micah` außerhalb von NixOS. Unter NixOS importierst du den Flake und fügst `inputs.micah.packages.${pkgs.system}.micah` zu `environment.systemPackages` hinzu. Für eine einfachere Einrichtung ist auch `nixosModules.micah` verfügbar.
- **AppImage:** Benötigt FUSE. Ohne FUSE: `./Micah_*.AppImage --appimage-extract-and-run`. Bei Darstellungsfehlern unter Wayland hilft möglicherweise `WEBKIT_DISABLE_DMABUF_RENDERER=1`. Die `.deb`- / `.rpm`-Pakete binden stattdessen den GTK-Stack des Systems ein und laufen meist flüssiger.

## KI konfigurieren

1. Öffne **Einstellungen -> KI**.
2. Wähle einen Anbieter und füge deinen API-Schlüssel ein. Für lokale Inferenz verweist du Micah auf deinen LM Studio- / MLX- / Ollama-Endpunkt.
...
Micah steht unter der Apache-2.0-Lizenz. Weitere Informationen zu unseren Abhängigkeiten findest du in der [Apache License 2.0](../../LICENSE).
