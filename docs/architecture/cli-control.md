...
This guide elaborates on `MICAH.md`. If anything here conflicts with `MICAH.md`, `MICAH.md` wins.

## Current surface

Micah bundles a small Rust client internally named `micah-cli`. Native terminal panes expose it as the public `micah` command:

```text
micah <file> [--line <n>] [--no-focus] [--json]
micah open <file> [--line <n>] [--no-focus] [--json]
micah ping [--json]
micah capabilities [--json]
micah identify [--json]
```

The app must already be running. A command launched inside a Micah pane targets that pane's space, even if another space or tab has UI focus. An external client without pane context falls back to the active UI context.

## Components

- `src-tauri/crates/micah-control-protocol` contains dependency-light request, response, error, descriptor, and method types shared by the app and CLI.
- `src-tauri/crates/micah-cli` contains the standalone console client. It intentionally avoids Clap, Tokio, reqwest, and Tauri.
...
The app binds an ephemeral loopback TCP port and writes a discovery descriptor to the user cache directory under `micah/control.json`. The descriptor contains the protocol version, address, process id, app version, and a random 256-bit token.
...
- The CLI verifies that a cache descriptor still names a live Micah process before sending its token.
...
The token is injected into Micah-spawned native shells together with `MICAH_PANE_ID`. Child coding agents inherit the caller context intentionally, which gives them the same local UI-control capability as the terminal that launched them. Tokens must never be logged or added to command output.

## Command discovery inside a PTY

The packaged helper is named `micah-cli` because a macOS app bundle places sidecars beside the GUI executable, which is already named `micah`. At app startup Micah creates a user-private, per-process `bin/micah` hard link to the packaged helper, falling back to a symlink on Unix or a copy on Windows. That directory is prepended to the PTY `PATH`.

Existing Bash, Zsh, Fish, and PowerShell integration also defines an interactive `micah` function that executes `$MICAH_CLI`. The real PATH entry is still required because non-interactive child shells do not reliably inherit shell functions.
...
`tauri.conf.json` declares `binaries/micah-cli` as an external binary. Tauri selects the file with the current target-triple suffix and signs or packages it with the app. The release workflow passes its explicit Rust target to `scripts/build-cli.mjs`, which prevents an x86_64 macOS release from accidentally bundling an arm64 helper from the host.
...
- Micah does not yet install a global command for external Terminal.app, PowerShell, or other terminals. The bundled helper and cache descriptor already support that future installer step.
- The CLI does not launch a stopped Micah app yet.
...
1. Add the method constant and typed parameters to `micah-control-protocol`.
...
- [`MICAH.md`](../../MICAH.md) - architecture source of truth
