...
2. Builds a stable system prompt from `selectSystemPrompt(modelId)` plus optional persona, custom instructions, and `MICAH.md` project memory.
...
Conversations are organized into sessions. Persistence lives in `micah-ai-sessions.json` via `tauri-plugin-store` (`src/modules/ai/lib/sessions.ts`):
