...
Micah runs shells, reads and writes files, and sends data to AI providers. The security model is defense-in-depth: no single guard is enough, so every boundary validates input before acting on it.
...
Local LLM endpoints are explicitly allowed because the user opted in by pointing Micah at them, but they are still classified and logged.
...
Service constant: `micah-ai`. Keys never touch disk outside the keychain/Linux secrets file, never go in `localStorage`, and never appear in logs.
...
The agent detector (`src-tauri/src/modules/pty/agent_detect.rs`) is armed by `OSC 133;C;<cmd>` or by a self-armed marker and emits `micah:agent-signal` events. It is driven **only by OSC sequences**, never by raw output, so a repainting TUI never flaps.
