...
Micah is two processes: the Rust backend (`src-tauri/`) and the webview frontend (`src/`).
...
- `secrets_get` / `secrets_set` / `secrets_delete` / `secrets_get_all` - OS keychain access, service `micah-ai`
