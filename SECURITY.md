...
Micah runs shells, reads/writes files, and talks to AI providers, so security bugs matter. If you find one, please tell us before posting it publicly.

## Reporting

Open a private security advisory at https://github.com/larteragia/micah/security/advisories/new. Include:
...
Until `1.0.0`, only the latest minor gets security fixes. See the current version in `package.json` or on the [Releases page](https://github.com/larteragia/micah/releases). 
...
- Release artifacts published on GitHub
- The auto-updater
...
- **No telemetry.** Micah only talks to the network when you ask it to (AI requests, update checks, web preview).
...
- Micah runs whatever you (or the agent) tell it to run, with your permissions. That's kind of the point of a terminal.
- AI providers see whatever you send them. Read their retention policies.
- Local LLM endpoints (LM Studio, OpenAI-compatible) are trusted at the network level - only point Micah at servers you control.
