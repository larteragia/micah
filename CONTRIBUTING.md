# Contributing to Micah

Micah is a solo-maintained project with a strong product direction. Contributions are welcome, but **alignment matters more than volume**.
...
- Micah has one active maintainer ([@larteragia](https://github.com/larteragia)).
...
For the architecture and how to contribute safely, see [MICAH.md](MICAH.md) and the [docs/ index](docs/README.md).

## Where to discuss


Use GitHub Discussions for design questions, scope questions, "should I work on X?" and quick feedback. Use GitHub Issues for tracking concrete bugs and features.
...
- **Pre-discussed features** - alignment in an issue or a GitHub Discussion first.
...
Micah positions itself as **lightweight, fast, production-grade**. Every PR is reviewed against:
...
If you're not sure how to measure perf or what counts as a hot path, ask in an issue or a GitHub Discussion. Better to confirm than get bounced.

## Changes to core subsystems require a test

The most common way a PR breaks Micah is a **local fix with global blast radius**: the diff solves one reported case, reads fine, passes type-check and clippy, and silently breaks the same subsystem in every other case. Review alone does not catch these. A test does.
...
The bar for the test is real coverage of the contract, not a placeholder. Test the case that would actually break: the edge, the deny path, the "what happens one level above home". If you can't see how to test it, ask in an issue before opening the PR. That conversation is usually shorter than the revert.
...
- Micah is terminal-first, not an IDE clone. Focused capabilities such as LSP, AI autocomplete, formatting, source control, and previews belong when they stay fast, lazy, and resource-bounded.
...
A: Open a GitHub issue or bring it to a GitHub Discussion. Don't open a PR without prior discussion.
