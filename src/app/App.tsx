...
export default function App() {
  ...
  const handleAttachFileToAgent = useCallback(
    (path: string) => {
      ...
      window.dispatchEvent(
        new CustomEvent<string>("micah:ai-attach-file", { detail: path }),
      );
      openPanel();
      focusInput(null);
    },
    ...
    (request: AgentLaunchRequest) => {
      ...
      const hooksReady = launcher.supportsHooks
        ? invoke("agent_enable_hooks", {
            ...
            console.warn(
              `[micah] could not enable ${request.agent} notifications:`,
              error,
            );
          })
        : Promise.resolve();
      ...
      for (const leafId of agentLeafIds) {
        void (async () => {
          ...
          if (!writeToSession(leafId, `${command.command}\r`)) {
            console.error(
              `[micah] agent terminal ${leafId} closed before launch`,
            );
          }
        })();
      }
  ...
  useEffect(() => {
    ...
    let disposed = false;
    (async () => {
      const off = await listen<string[]>("micah:open-file", (e) => {
  ...
  const shortcutHandlers = useMemo<ShortcutHandlers>(
    () => ({
      ...
      "window.new": () => {
        // Read through the refs: this runs on keypress, so it needs no deps.
        const tab = tabsRef.current.find((t) => t.id === activeIdRef.current);
        void openNewWindow(tab?.kind === "terminal" ? tab.cwd : null);
      },
      "tab.new": openNewTab,
    ...
    () =>
      commandPaletteOpen
        ? createCommandItems({
            ...
            home,
            openNewTab,
            openNewWindow: () => void openNewWindow(activeCwd),
            ...
            toggleBrowser: () => browser.setEnabled(!browser.enabled),
            ...
            >
              ...
              >
                <div className="flex h-full min-h-0 flex-col border-l border-border/60 bg-card">
                  <div
                    key={sidebarView}
                    className="min-h-0 flex-1 micah-panel-in"
                  >
                    {sidebarView === "explorer" ? (
                      ...
                      <TabsPanel
                        ...
                        onNew={openNewTab}
                        onNewWindow={() => void openNewWindow(activeCwd)}
