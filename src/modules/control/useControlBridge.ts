...
async function respond(
  ...
  if (!delivered) {
    console.warn(`[micah] control response expired: ${requestId}`);
  }
}
  ...
  useEffect(() => {
    ...
    const handleRequest = async (request: ControlRequest) => {
      try {
        ...
        if (request.method === "open") {
          ...
          if (open.focus) {
            ...
            try {
              ...
              console.warn("[micah] could not focus control target:", error);
            }
          }
          ...
          if (tabId === null) {
            throw new RequestError(
              "open_failed",
              "Micah could not create an editor tab",
            );
          }
        ...
        await respond(request.id, { ok: false, error: responseError }).catch(
          (responseError) => {
            console.error("[micah] control response failed:", responseError);
          },
        );
      }
    };

    void listen<ControlRequest>("micah:control-request", (event) => {
      ...
      .catch((error) => {
        console.error("[micah] control bridge setup failed:", error);
      });
    ...
    return () => {
      ...
      void setFrontendReady(false).catch((error) => {
        console.error("[micah] control bridge cleanup failed:", error);
      });
    };
