...
function distroCommand(key: DistroKey, version: string): string {
  switch (key) {
    case "arch":
      return "yay -S micah-bin";
    case "debian":
      return `sudo apt install ./Micah_${version}_amd64.deb`;
    case "fedora":
      return `sudo dnf install ./Micah-${version}-1.x86_64.rpm`;
  }
}
          ...
          <DialogTitle>
            {ready
              ...
              : downloading
                ? "Downloading update…"
                : manual
                  ? `Micah v${manual.version} is available`
                  : `Micah v${update?.version} is available`}
          </DialogTitle>
          <DialogDescription>
            {ready
              ? "Restart Micah to finish installing."
              : downloading
                ? progress !== null
