...
import type { MicahLspClient } from "./client";
...
async function createSession(
  ...
  const [{ TauriLspTransport }, { MicahLspClient }] = await Promise.all([
    import("./transport"),
    import("./client"),
  ]);

  if (MicahLspClient.hostPid === null) {
    MicahLspClient.hostPid = await invoke<number>("lsp_host_pid").catch(
      () => null,
    );
  }
  ...
  const client = new MicahLspClient({
    transport,
    rootUri,
