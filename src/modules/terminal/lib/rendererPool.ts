...
function getRecycler(): HTMLDivElement {
  ...
  el.setAttribute("data-micah-recycler", "");
  el.style.cssText =
...
function createSlot(): Slot {
  ...
  host.setAttribute("data-micah-slot", String(slots.length));
  ...
  if (!fast) {
    ...
    if (p.snapshot) {
      try {
        ...
        console.warn("[micah] snapshot replay failed:", e);
      }
    }
    if (p.altScreen) {
...
function serializeSlot(slot: Slot): SerializeOutput {
  ...
  try {
    ...
    console.warn("[micah] serialize failed:", e);
  }
  return {
    snapshot,
    cols: slot.term.cols,
...
function disposeSlot(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[micah] slot dispose failed:", e);
  }
  slot.host.remove();
...
function attachWebgl(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[micah-webgl] unavailable:", e);
  }
}
...
function disposeSlotWebgl(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[micah-webgl] dispose failed:", e);
  }
  try {
    const r = (
