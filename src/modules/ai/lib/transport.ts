...
const MICAH_MD_MAX_BYTES = 32 * 1024;
type MemoryCacheEntry = { content: string | null; mtime: number };
const projectMemoryCache = new Map<string, MemoryCacheEntry>();

async function readMicahMd(workspaceRoot: string | null): Promise<string | null> {
  if (!workspaceRoot) return null;
  const path = `${workspaceRoot.replace(/\/$/, "")}/MICAH.md`;
  ...
  try {
    ...
    const content =
      r.content.length > MICAH_MD_MAX_BYTES
        ? r.content.slice(0, MICAH_MD_MAX_BYTES)
        : r.content;
...
type Deps = {
  ...
  getClaudeCodeSkipPermissions?: () => boolean;
...
export function createContextAwareTransport(deps: Deps) {
  const run = async (options: SendOptions) => {
    ...
    const projectMemory = await readMicahMd(live.workspaceRoot);
    ...
    const result = await runAgentStream({
      ...
      claudeCodeCwd: live.workspaceRoot ?? live.cwd,
      claudeCodeSkipPermissions: deps.getClaudeCodeSkipPermissions?.(),
