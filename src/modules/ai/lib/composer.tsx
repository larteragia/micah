...
export function AiComposerProvider({ children }: ProviderProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("micah:ai-attach-file", onAttach);
    return () => window.removeEventListener("micah:ai-attach-file", onAttach);
  ...
  const submit = () => {
    ...
    // the prompt to the MICAH.md scan template before sending.
    ...
    if (commandSource.startsWith("/") || commandSource.startsWith("#")) {
      ...
      if (outcome.kind === "send-prompt") {
        ...
        if (outcome.commandName) {
          commandMarker = `<micah-command name="${outcome.commandName}" />`;
        }
      }
    }
