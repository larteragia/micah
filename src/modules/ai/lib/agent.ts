...
export type BuildModelOptions = {
  ...
  /** Directory the Claude Code CLI runs in — its tools are rooted there. */
  claudeCodeCwd?: string | null;
  claudeCodePermissionMode?: "default" | "acceptEdits" | "plan" | "bypassPermissions";
...
export async function buildLanguageModel(
  ...
  if (provider === "claude-code") {
    // Never cached: cwd and permission mode change between turns, and the
    // model object is a thin wrapper around a process spawn anyway.
    const { createClaudeCodeModel } = await import("@/cavalo/claudeCode");
    return createClaudeCodeModel(resolvedModelId.replace(/^cc-/, ""), {
      getCwd: () => options.claudeCodeCwd ?? null,
      getPermissionMode: () =>
        options.claudeCodePermissionMode ?? "acceptEdits",
      getSkipPermissions: () => options.claudeCodeSkipPermissions ?? false,
...
export type LocalProviderConfig = {
  ...
  claudeCodeCwd?: string | null;
  claudeCodePermissionMode?: BuildModelOptions["claudeCodePermissionMode"];
  claudeCodeSkipPermissions?: boolean;
};
...
export function buildConfiguredLanguageModel(
  ...
  return buildLanguageModel(m.provider, keys, resolvedId, {
    ...
    claudeCodeCwd: local.claudeCodeCwd,
    claudeCodePermissionMode: local.claudeCodePermissionMode,
    claudeCodeSkipPermissions: local.claudeCodeSkipPermissions,
  });
}
...
function buildStableSystem(
  ...
  // Claude Code arrives with its own system prompt and its own tools. Sending
  // Micah's on top would describe tools it does not have and re-bill a few
  // thousand tokens of instructions it will ignore.
  const base =
    provider === "claude-code" ? "" : selectSystemPrompt(modelId);
  ...
  const memoryBlock =
    projectMemory && projectMemory.trim().length > 0
      ? `\n\n## PROJECT — MICAH.md\n${projectMemory.trim()}`
      : "";
  return `${base}${memoryBlock}${personaBlock}${customBlock}`.trim();
}
...
export async function runAgentStream(opts: RunAgentOptions) {
  ...
  const model = await buildConfiguredLanguageModel(modelId, opts.keys, {
    ...
    claudeCodeCwd: opts.claudeCodeCwd,
    // Micah's plan mode maps onto the CLI's own read-only planning mode.
    claudeCodePermissionMode: opts.planMode ? "plan" : "acceptEdits",
    claudeCodeSkipPermissions: opts.claudeCodeSkipPermissions,
  });
  ...
  const prompt = prepareAgentPrompt(
    stableSystem,
    // The CLI has its own plan mode; it is switched on with --permission-mode
    // rather than argued for in prose.
    opts.planMode && !isClaudeCode ? PLAN_MODE_PROMPT : null,
    compactedHistory,
    provider,
  );
  ...
  return streamText({
    ...
    // Claude Code executes its own tools inside its own loop. Handing it
    // Micah's set as well would mean two agents fighting over one task.
    tools: isClaudeCode ? undefined : buildTools(opts.toolContext),
    stopWhen: stepCountIs(isClaudeCode ? 1 : MAX_AGENT_STEPS),
