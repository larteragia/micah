...
const INIT_PROMPT = `Scan this workspace and produce MICAH.md at the workspace root with:
...
Use grep/glob/list_directory/read_file to explore. Cap MICAH.md under 200 lines. Use write_file to create it (will go through normal approval).`;
...
export const MICAH_CMD_RE =
  /^<micah-command\s+name="([a-z0-9-]+)"(?:\s+state="([a-z]+)")?\s*\/>(?:\n+|$)/;

export function wrapWithCommandMarker(prompt: string, name: string): string {
  return `<micah-command name="${name}" />\n\n${prompt}`;
}
