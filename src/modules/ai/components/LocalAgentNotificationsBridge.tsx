...
export function LocalAgentNotificationsBridge() {
  ...
  useEffect(() => {
    ...
    if (status === "awaiting-approval") {
      fire("attention", "Micah needs your approval", "Approve a tool to continue");
    } else if (status === "error") {
      fire("error", "Micah run failed", error ?? undefined);
    } else if (status === "idle" && isBusy(was)) {
      fire("finished", "Micah finished", "Your task is ready");
    }
  }, [status, error]);
