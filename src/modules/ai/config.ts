export const KEYRING_SERVICE = "micah-ai";

// OpenRouter is deliberately absent and must stay absent. Micah bills nothing
// per token; a gateway that resells metered capacity has no place in the enum,
// because anything present here eventually gets configured by someone.
...
export const PROVIDERS: readonly ProviderInfo[] = [
  {
    ...
    keyringAccount: "",
    keyPrefix: null,
    consoleUrl: "https://claude.com/product/claude-code",
  },
  {
    id: "openai",
    label: "OpenAI",
...
export const MODELS = [
  // ── Claude Code (local CLI, user's own subscription, no API key) ──────────
  // The id after `cc-` is handed straight to `claude --model`.
  {
    id: "cc-opus",
    provider: "claude-code",
    label: "Claude Code · Opus",
    hint: "Subscription",
    description: "Runs the local `claude` CLI on your plan. No API key.",
  ...
  },
  {
    ...
    hint: "Subscription",
    description: "Balanced Claude Code agent on your plan. No API key.",
  ...
  },
  {
    ...
    hint: "Subscription",
    description: "Fastest Claude Code agent on your plan. No API key.",
...
// The default is the one backend that costs nothing per token: the user's own
// Claude Code subscription. Everything else has to be chosen deliberately.
export const DEFAULT_MODEL_ID: ModelId = "cc-sonnet";
...
export const KEYLESS_PROVIDERS: readonly ProviderId[] = [
  // Claude Code authenticates through its own CLI login — Micah never sees,
  // stores or forwards a credential for it.
  "claude-code",
  "lmstudio",
  "mlx",
  "ollama",
 ...
 *  Claude Code is excluded on purpose: every completion would spawn a CLI
 *  process and burn a full agent turn on a few characters of code. */
...
export const SYSTEM_PROMPT = `You are Micah, an AI agent embedded in a developer terminal emulator. You are a hands-on engineer, not a chat bot — your job is to *do* the work, not narrate it.
...
export const SYSTEM_PROMPT_LITE = `You are Micah, an AI agent in a developer terminal. Each turn carries an <env> block (workspace_root, active_terminal_cwd, optional active_file) prepended to the user's message — treat as ground truth.
