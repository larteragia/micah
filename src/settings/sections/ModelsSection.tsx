...
export function ModelsSection() {
  ...
  const claudeCodeSkipPermissions = usePreferencesStore(
    (s) => s.claudeCodeSkipPermissions,
  );
  ...
  const isConfigured = (id: ProviderId): boolean => {
    // Claude Code needs nothing configured here — it is connected as soon as
    // the CLI is installed and logged in, which is checked in its own card.
  ...
  return (
    <div className="flex flex-col gap-7">
      <SectionHeader
        title="Models"
        description="Connect the providers you use. Keys live in your OS keychain and are used only by Micah."
      />
            ...
            {visibleProviders.map((p) =>
              p.id === "claude-code" ? (
                <ClaudeCodeCard
                  key={p.id}
                  provider={p}
                  skipPermissions={claudeCodeSkipPermissions}
                  onSkipPermissionsChange={(v) =>
                    void setClaudeCodeSkipPermissions(v)
                  }
                />
...
function ClaudeCodeCard({
  ...
  const [probe, setProbe] = useState<
    { state: "checking" } | { state: "ok"; version: string } | { state: "missing" }
  >({ state: "checking" });

  useEffect(() => {
    let alive = true;
    void native
      .runCommand("claude --version", null, 20)
      .then((r) => {
        if (!alive) return;
        const version = (r.stdout || "").trim().split(/\r?\n/)[0] ?? "";
        setProbe(
          r.exit_code === 0 && version
            ? { state: "ok", version }
            : { state: "missing" },
        );
      })
  ...
  return (
    <div className="flex flex-col gap-2 rounded-lg border border-border/60 bg-card/60 px-3 py-2.5">
      ...
      <span className="text-[10.5px] leading-relaxed text-muted-foreground">
        Runs the local <span className="font-mono">claude</span> CLI on your own
        subscription. No API key is stored, sent or needed — Claude Code signs
        itself in.
      </span>

      {probe.state === "missing" ? (
        <span className="text-[10.5px] leading-relaxed text-destructive">
          <span className="font-mono">claude</span> was not found on PATH.
          Install Claude Code, then run <span className="font-mono">claude</span>{" "}
          once to sign in.
        </span>
      ) : null}
      ...
      <div className="mt-0.5 flex items-start justify-between gap-4">
        <div className="flex flex-col">
          <span className="text-[11.5px]">Skip permission prompts</span>
          <span className="text-[10.5px] leading-relaxed text-muted-foreground">
            Passes --dangerously-skip-permissions. Without it the CLI can edit
            files but refuses commands it would normally ask about, because a
            background run has nobody to ask.
          </span>
        </div>
        <Switch
