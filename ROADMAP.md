...
- Not an IDE clone. Micah selectively integrates high-value editor capabilities such as LSP, AI autocomplete, formatting, source control, and previews without adopting the heavyweight runtime and always-on background services of a traditional IDE.
...
Strategic areas where help is welcome. Pick something and propose an approach in a GitHub Discussion or via an issue first.
...
Categories that will not be built into Micah. Individual feature requests in these categories will be closed.
...
- **Notebook and document workspaces.** Anything that turns Micah into a document host rather than a terminal.
...
- **Telemetry, analytics, accounts.** Micah stays BYOK and offline-respectful.
...
- **Third-party subscription session bridges.** Forwarding cloud subscription auth (provider-managed login sessions) through Micah is not technically feasible for third-party clients.

## Decision authority

Direction and scope decisions are made by [@larteragia](https://github.com/larteragia). Trusted reviewers (informal, no fixed roles yet) provide input on security, performance, and platform-specific areas.
