...
use micah_lib::modules::fs::grep::{fs_glob_blocking, fs_grep_blocking};
use micah_lib::modules::fs::search::{fs_list_files_blocking, fs_search_blocking};
use micah_lib::modules::fs::tree::{fs_read_dir_blocking, list_subdirs_blocking, EntryKind};
...
fn list_files_returns_sorted_relative_paths() {
    ...
    let res = fs_list_files_blocking(fx.root_str(), None, None, None, None).expect("list");
...
fn list_files_max_depth_clamps() {
    ...
    let res = fs_list_files_blocking(fx.root_str(), None, Some(1), None, None).expect("list");
...
fn list_files_non_dir_errors() {
    assert!(fs_list_files_blocking("/no/such/dir".into(), None, None, None, None).is_err());
}
...
fn read_dir_orders_dirs_before_files_then_alpha() {
    ...
    let entries = fs_read_dir_blocking(fx.root_str(), false, None, None).expect("read_dir");
...
fn read_dir_hides_dotfiles_by_default() {
    ...
    let hidden_off = fs_read_dir_blocking(fx.root_str(), false, None, None).expect("read_dir");
    let names: Vec<&str> = hidden_off.iter().map(|e| e.name.as_str()).collect();
    assert_eq!(names, vec!["visible.txt"]);

    let hidden_on = fs_read_dir_blocking(fx.root_str(), true, None, None).expect("read_dir");
...
fn read_dir_flags_gitignored_entries_only_when_requested() {
    ...
    let entries = fs_read_dir_blocking(fx.repo_str(), false, Some(true), None).expect("read_dir");
    let flag = |name: &str| {
        entries
            .iter()
            .find(|e| e.name == name)
    ...
    let plain = fs_read_dir_blocking(fx.repo_str(), false, None, None).expect("read_dir");
...
fn read_dir_skips_gitignore_outside_a_repo() {
    ...
    fx.write("kept.txt", "");
    let entries = fs_read_dir_blocking(fx.root_str(), false, Some(true), None).expect("read_dir");
...
fn list_subdirs_returns_only_directories() {
    ...
    let dirs = list_subdirs_blocking(fx.root_str(), false, None).expect("list_subdirs");
...
fn list_subdirs_hides_dot_dirs_by_default() {
    ...
    let off = list_subdirs_blocking(fx.root_str(), false, None).expect("list_subdirs");
    assert_eq!(off, vec!["visible"]);

    let on = list_subdirs_blocking(fx.root_str(), true, None).expect("list_subdirs");
