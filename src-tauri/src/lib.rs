...
async fn open_settings_window(app: tauri::AppHandle, tab: Option<String>) -> Result<(), String> {
    ...
    if let Some(window) = app.get_webview_window("settings") {
        ...
        if let Some(t) = tab.as_deref().filter(|s| !s.is_empty()) {
            ...
            let _ = window.emit("micah:settings-tab", t);
        }
        return Ok(());
    }
    ...
    // Named on every platform, read back only on Linux below.
...
/// Open another main-window instance in this same process. Labels are
/// "main", "main-2", "main-3"… — the capability files match them with "main*",
/// and pty/control state is keyed by label so windows never reap each other.
#[tauri::command]
async fn open_new_window(app: tauri::AppHandle, cwd: Option<String>) -> Result<String, String> {
    let label = (2..1000)
        .map(|n| format!("main-{n}"))
        .find(|candidate| app.get_webview_window(candidate).is_none())
        .ok_or_else(|| "too many windows".to_string())?;

    // Seed before the webview boots: it drains this on first paint, so a
    // window opened from a workspace starts there instead of at the launch dir.
    if let Some(dir) = cwd.as_deref().filter(|d| !d.is_empty()) {
        ...
        if let Some(state) = app.try_state::<LaunchDir>() {
            *state.0.lock().expect("LaunchDir mutex poisoned") = Some(dir.to_string());
        }
    }
    ...
    let builder = WebviewWindowBuilder::new(&app, &label, WebviewUrl::App("index.html".into()))
        ...
        .resizable(true)
        // Same as the main window: stay hidden until the frontend paints, so
        // no transparent shadow-frame flash.
        .visible(false);
    ...
    // Linux/Windows render their own titlebar.
    #[cfg(any(target_os = "linux", target_os = "windows"))]
    let builder = builder.decorations(false).transparent(true);
    ...
    // Offset from the window that spawned it so it doesn't land exactly on top.
    if let Some(main) = app.get_webview_window("main") {
...
pub fn run() {
    #[cfg(windows)]
    {
        ...
        if args.get(1).map(String::as_str) == Some("__micah_notify") {
    ...
    builder
        ...
        .setup(move |_app| {
            // Logged at boot so "what is actually running" is answerable from the
            // log file alone, without asking the app nicely over IPC.
            log::info!("micah build {}", env!("MICAH_BUILD_ID"));
            if let Err(error) = control::start(_app.handle().clone(), control_for_setup.clone()) {
                log::warn!("could not start Micah control server: {error}");
            }
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            fs::claude_session::claude_sessions_recent,
        ...
        .run(|app, event| {
            match event {
                ...
                tauri::RunEvent::Exit => {
                    ...
                    // would then drive a stranger.
                    browser::clear_discovery(app);
                }
                // A closed window's shells would otherwise outlive it: the
                // frontend that owned them is gone, so nothing else reaps them.
                tauri::RunEvent::WindowEvent {
                    label,
                    event: tauri::WindowEvent::Destroyed,
                    ..
                } => {
                    if let Some(state) = app.try_state::<pty::PtyState>() {
                        let reaped = pty::close_window_sessions(&state, &label);
                        if reaped > 0 {
                            log::info!("window {label} closed: reaped {reaped} pty session(s)");
                        }
                    }
                    if let Some(state) = app.try_state::<control::ControlState>() {
                        state.drop_window(&label);
                    }
                    // The OS destroys the child webview with its host window, but
                    // no frontend cleanup runs — without this the panel stays
                    // claimed by a window that no longer exists.
                    if let Some(state) = app.try_state::<browser::BrowserState>() {
                ...
                tauri::RunEvent::Opened { urls } => {
                    ...
                    let _ = app.emit("micah:open-file", target.files);
                }
                _ => {}
            }
        });
