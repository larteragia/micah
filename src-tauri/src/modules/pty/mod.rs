...
pub struct PtyState {
    ...
    // Which window opened each session. Sessions are process-global but every
    // window owns only its own: without this, a second window booting would
    // reap the first window's shells in pty_close_all.
...
/// Kill every session opened by `label`, detaching each drop onto its own
/// thread (see pty_close for why). Returns how many were reaped.
pub fn close_window_sessions(state: &PtyState, label: &str) -> usize {
    let ids: Vec<u32> = {
    ...
    let mut drained: Vec<(u32, Arc<Session>)> = Vec::with_capacity(ids.len());
    {
    ...
    for (id, s) in drained {
        if let Err(e) = s.killer.lock().unwrap().kill() {
            log::debug!("close_window_sessions: kill id={id} returned {e}");
        }
        thread::Builder::new()
            .name(format!("micah-pty-drop-{id}"))
...
pub async fn pty_open(
    ...
    // Route control-protocol requests from this shell back to the window that
    // owns the pane instead of always to "main".
    if let Some(pane_id) = pane_id {
        control.bind_pane(pane_id, &owner);
    }
    ...
    state.owners.write().unwrap().insert(id, owner);
...
pub fn pty_close(state: tauri::State<PtyState>, id: u32) -> Result<(), String> {
    state.owners.write().unwrap().remove(&id);
...
// calling window so booting a second window leaves the first one's shells.
#[tauri::command]
pub fn pty_close_all(window: tauri::Window, state: tauri::State<PtyState>) -> Result<usize, String> {
    let label = window.label();
    let count = close_window_sessions(&state, label);
    if count > 0 {
        log::info!("pty_close_all: reaped {count} orphaned session(s) of window {label}");
    }
    Ok(count)
}
