# micah-shell-integration (PowerShell)
...
if ($global:__MICAH_HOOKS_LOADED) { return }
$global:__MICAH_HOOKS_LOADED = $true

if ($env:MICAH_CLI -and (Test-Path -LiteralPath $env:MICAH_CLI -PathType Leaf)) {
    function global:micah {
        & $env:MICAH_CLI @args
    }
}
...
if (Test-Path Function:prompt) {
    Copy-Item Function:prompt Function:__micah_user_prompt -Force -ErrorAction SilentlyContinue
}

function global:__micah_urlencode {
    param([string]$s)
...
function global:__micah_install_readline {
    if ($global:__micah_readline_done) { return }
    if (-not (Test-Path Function:PSConsoleHostReadLine)) { return }
    $global:__micah_readline_done = $true
    ...
    Copy-Item Function:PSConsoleHostReadLine Function:global:__micah_user_readline -Force
    function global:PSConsoleHostReadLine {
        try {
            $line = __micah_user_readline
        } catch {
            # Self-heal: restore the original reader so a broken wrapper can
            # never lock the shell out of input.
            Copy-Item Function:__micah_user_readline Function:global:PSConsoleHostReadLine -Force
            return ''
        }
        try {
            if ($line -is [string] -and $line.Trim().Length -gt 0) {
                $global:__micah_block_seen = $true
                $esc = [char]27
...
function global:prompt {
    ...
    if ($env:MICAH_BLOCKS) {
        ...
        $gap = if ($global:__micah_block_seen) { "`n`n" } else { "`n" }
        return "$oscD$oscA$osc7$gap$oscB"
    }

    $original = if (Test-Path Function:__micah_user_prompt) {
        try { & __micah_user_prompt } catch { "PS $((Get-Location).Path)> " }
    } else {
