...
const FISH_REINSTALL_PROMPT: &str =
    "functions -q __micah_install_prompt; and __micah_install_prompt";
...
pub struct ShellInfo {
    pub name: String,
    pub path: String,
    /// True when Micah injects OSC 7/133 integration for this shell (cwd
...
fn apply_common(
    ...
    cmd.env("MICAH_TERMINAL", "1");
    if blocks {
        cmd.env("MICAH_BLOCKS", "1");
    }
    ...
    if let Some(control) = control {
        cmd.env("MICAH_CONTROL_ADDR", &control.address);
        cmd.env("MICAH_CONTROL_TOKEN", &control.token);
        cmd.env("MICAH_PANE_ID", control.pane_id.to_string());
        if let Some(path) = &control.cli_path {
            cmd.env("MICAH_CLI", path);
        }
...
mod unix {
    ...
    fn integration_root() -> Result<PathBuf, String> {
        ...
        let root = home.join(".cache").join("micah").join("shell-integration");
    ...
    fn prepare_fish_conf_d() -> Result<(), String> {
        ...
        fs::create_dir_all(&dir).map_err(|e| format!("create {}: {e}", dir.display()))?;
        write_if_changed(&dir.join("micah.fish"), FISH_INIT)?;
        Ok(())
    }
...
mod windows {
        ...
        match (shell_kind, integration) {
            (
                ...
                if let Some(user_zdotdir) = user_zdotdir {
                    args.push(format!("MICAH_USER_ZDOTDIR={user_zdotdir}"));
                }
    ...
    fn prepare_wsl_integration_dir(distro: &str, shell: &str) -> Result<(String, PathBuf), String> {
        ...
        let linux_dir = format!(
            "{}/.cache/micah/shell-integration/{shell}",
    ...
    fn prepare_wsl_fish_conf_d(distro: &str) -> Result<(), String> {
        ...
        let unc_dir = crate::modules::workspace::wsl_path_to_unc(distro, &linux_dir);
        fs::create_dir_all(&unc_dir).map_err(|e| format!("create {}: {e}", unc_dir.display()))?;
        let unc_file = unc_dir.join("micah.fish");
    ...
    mod tests {
        ...
        fn builds_wsl_zsh_launch_spec_with_env_and_login() {
            let spec = build_wsl_launch_spec(
                ...
                ShellKind::Zsh,
                WslShellIntegration::Zsh {
                    zdotdir: "/home/vinicios/.cache/micah/shell-integration/zsh".into(),
                    user_zdotdir: None,
                },
            );
            assert_eq!(
                ...
                vec![
                    ...
                    "--exec".to_string(),
                    "env".to_string(),
                    "ZDOTDIR=/home/vinicios/.cache/micah/shell-integration/zsh".to_string(),
        ...
        fn builds_wsl_zsh_launch_spec_with_user_zdotdir_probe() {
            ...
            assert_eq!(
                ...
                vec![
                    ...
                    "--exec".to_string(),
                    "env".to_string(),
                    "MICAH_USER_ZDOTDIR=/home/vinicios/.config/zsh".to_string(),
                    "ZDOTDIR=/home/vinicios/.cache/micah/shell-integration/zsh".to_string(),
                    "/usr/bin/zsh".to_string(),
                    "-l".to_string(),
                ]
        ...
        fn builds_wsl_bash_launch_spec_with_rcfile() {
            let spec = build_wsl_launch_spec(
                ...
                WslShellIntegration::Bash {
                    rcfile: "/home/vinicios/.cache/micah/shell-integration/bash/bashrc".into(),
                },
            );
            assert_eq!(
                spec.args,
                vec![
                    ...
                    "/bin/bash".to_string(),
                    "--rcfile".to_string(),
                    "/home/vinicios/.cache/micah/shell-integration/bash/bashrc".to_string(),
                    "-i".to_string(),
                ]
            );
        }
...
mod tests {
    ...
    fn common_env_includes_authenticated_caller_context() {
        ...
        let control = ShellControlEnv {
            ...
            token: "secret".into(),
            pane_id: 42,
            cli_path: Some("/app/micah-cli".into()),
            cli_bin_dir: Some(std::path::PathBuf::from("/app/bin")),
        };
        apply_common(&mut command, None, false, Some(&control));

        assert_eq!(
            command.get_env("MICAH_CONTROL_ADDR"),
            Some(OsStr::new("127.0.0.1:1234"))
        );
        assert_eq!(
            command.get_env("MICAH_CONTROL_TOKEN"),
            Some(OsStr::new("secret"))
        );
        assert_eq!(command.get_env("MICAH_PANE_ID"), Some(OsStr::new("42")));
        assert_eq!(
            command.get_env("MICAH_CLI"),
            Some(OsStr::new("/app/micah-cli"))
        );
        assert_eq!(
            command
                .get_env("PATH")
