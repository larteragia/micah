...
const PI_EXTENSION_FILE: &str = "micah-notifications.ts";
const PI_EXTENSION_MARKER: &str = "micah-pi-notifications-v1";
const PI_STATUS_NEEDLES: [&str; 6] = [
    PI_EXTENSION_MARKER,
    "agent_start",
    "agent_settled",
    "notify;Micah;pi;${event}",
    "emit(\"working\")",
    "emit(\"finished\")",
];
const PI_EXTENSION: &str = r#"// micah-pi-notifications-v1
import type { ExtensionAPI } from "@earendil-works/pi-coding-agent";

export default function (pi: ExtensionAPI) {
  const emit = (event: "working" | "finished") => {
    if (process.env.MICAH_TERMINAL) {
      process.stdout.write(`\u001b]777;notify;Micah;pi;${event}\u0007`);
    }
  };
...
const OWNED_MARKERS: [&str; 3] = ["notify;Micah;", "micah;notify", "__micah_notify"];
...
fn hook_command(spec: &AgentSpec, event: &str) -> String {
    match spec.delivery {
        Delivery::TerminalSequence => format!(
            r#"[ -n "$MICAH_TERMINAL" ] && printf '{{"terminalSequence":"\\u001b]777;notify;Micah;{event}\\u0007"}}' || true"#
        ),
...
fn osc_command(agent: &str, event: &str) -> String {
    format!(
        r#"[ -n "$MICAH_TERMINAL" ] && printf '\033]777;notify;Micah;{agent};{event}\007' > /dev/tty; printf '{{}}'"#
    )
}
...
fn osc_command(agent: &str, event: &str) -> String {
    let exe = std::env::current_exe()
        ...
        .unwrap_or_else(|_| "micah.exe".to_string());
    format!(r#""{exe}" __micah_notify {agent} {event}"#)
}
...
fn status_needle(spec: &AgentSpec, event: &str) -> String {
    match spec.delivery {
        Delivery::TerminalSequence => format!("notify;Micah;{event}"),
        Delivery::Osc => {
            #[cfg(unix)]
            {
                format!("notify;Micah;{};{event}", spec.agent)
            }
            #[cfg(windows)]
            {
                format!("__micah_notify {} {event}", spec.agent)
            }
        }
    }
}
...
fn pi_extension_contents(
    ...
    if existing.is_some_and(|s| !s.trim().is_empty() && !s.contains(PI_EXTENSION_MARKER)) {
        return Err(format!(
            "{} is not managed by Micah; refusing to overwrite",
            path.display()
        ));
    }
    Ok(PI_EXTENSION)
...
fn write_atomic(path: &std::path::Path, contents: &str) -> Result<(), String> {
    let tmp = path.with_extension("micah-tmp");
...
fn conout_marker(agent: &str, event: &str) -> String {
    format!("\x1b]777;notify;Micah;{agent};{event}\x07")
}

// Windows has no /dev/tty: the hook calls `micah.exe __micah_notify ...` and we
...
pub fn emit_conout_marker(agent: &str, event: &str) {
    ...
    if std::env::var_os("MICAH_TERMINAL").is_none() {
        return;
    }
    unsafe {
...
mod tests {
    ...
    fn claude_adds_all_event_hooks_to_empty_config() {
        ...
        assert!(command(&out, "Notification", 0).contains("notify;Micah;attention"));
        assert!(command(&out, "Stop", 0).contains("notify;Micah;finished"));
        assert!(command(&out, "UserPromptSubmit", 0).contains("notify;Micah;working"));
    ...
    fn conout_marker_matches_detector_format() {
        ...
        assert_eq!(
            ...
            "\u{1b}]777;notify;Micah;gemini;attention\u{7}"
        );
    }
    ...
    fn codex_emits_four_field_dev_tty_marker() {
        ...
        assert!(stop.contains("notify;Micah;codex;finished"));
    ...
    fn gemini_uses_matcher_and_named_marker() {
        ...
        assert!(command(&out, "AfterAgent", 0).contains("notify;Micah;gemini;finished"));
        assert!(command(&out, "Notification", 0).contains("notify;Micah;gemini;attention"));
    }

    #[test]
    fn pi_extension_emits_named_working_and_finished_markers() {
        let path = std::path::Path::new("/x/micah-notifications.ts");
        ...
        assert!(extension.contains("process.env.MICAH_TERMINAL"));
        assert!(extension.contains("process.stdout.write"));
    }

    #[test]
    fn pi_extension_only_replaces_micah_owned_file() {
        let path = std::path::Path::new("/x/micah-notifications.ts");
    ...
    fn pi_extension_install_is_atomic_idempotent_and_preserves_foreign_files() {
        let dir = std::env::temp_dir().join(format!("micah-pi-extension-{}", std::process::id()));
    ...
    fn pi_extension_install_preserves_symlink() {
        ...
        let dir =
            std::env::temp_dir().join(format!("micah-pi-extension-symlink-{}", std::process::id()));
        ...
        let legacy = json!({
            "hooks": {
                "Notification": [
                    { "hooks": [ {
                        "type": "command",
                        "command": "[ -n \"$MICAH_TERMINAL\" ] && printf '\\033]777;micah;notify\\033\\\\' > /dev/tty || true"
                    } ] }
                ]
            }
        });
