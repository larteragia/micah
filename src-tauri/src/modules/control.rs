...
const CONTROL_EVENT: &str = "micah:control-request";
...
struct ControlCore {
    ...
    // pane_id -> window label, so a request made inside a shell reaches the
    // window that owns that pane instead of always the first one.
    panes: Mutex<HashMap<u32, String>>,
}
...
impl ControlState {
    ...
    /// Remember which window owns a pane (called when its shell is spawned).
    pub fn bind_pane(&self, pane_id: u32, window_label: &str) {
        self.0
            .panes
            .lock()
            .expect("control panes poisoned")
            .insert(pane_id, window_label.to_string());
    }

    /// Forget every pane of a window that is going away.
    ...
    fn pane_window(&self, pane_id: Option<u32>) -> Option<String> {
        let pane_id = pane_id?;
        self.0
            .panes
            .lock()
...
pub fn start(app: tauri::AppHandle, state: ControlState) -> Result<(), String> {
    ...
    let launcher_dir = cli_path.as_deref().and_then(|cli_path| {
        match prepare_cli_launcher(&descriptor_path, cli_path) {
            Ok(dir) => Some(dir),
            Err(error) => {
                log::warn!("could not prepare micah CLI launcher: {error}");
                None
            }
        }
    });
    ...
    if cli_path.is_none() {
        log::warn!("bundled micah-cli executable not found; shell alias disabled");
    }

    let listener_state = state.clone();
    if let Err(error) = thread::Builder::new()
        .name("micah-control-listener".into())
...
fn accept_loop(listener: TcpListener, app: tauri::AppHandle, state: ControlState) {
    for incoming in listener.incoming() {
        ...
        if let Err(error) = thread::Builder::new()
            .name("micah-control-request".into())
...
fn forward_to_frontend(
    ...
    if !state.0.frontend_ready.load(Ordering::Acquire) {
        return ControlResponse::failure(
            request.id,
            "frontend_not_ready",
            "Micah is still restoring its workspace; try again shortly",
        );
    }
    ...
    let target = state
        .pane_window(request.caller.pane_id)
    ...
    if let Err(error) = app.emit_to(target.as_str(), CONTROL_EVENT, frontend_request) {
        state
            .0
            .pending
            .lock()
        ...
        return ControlResponse::failure(
            ...
            format!("could not reach Micah UI: {error}"),
        );
    }
    ...
    match receiver.recv_timeout(FRONTEND_TIMEOUT) {
        ...
        Err(mpsc::RecvTimeoutError::Timeout) => {
            state
                ...
                .expect("control pending poisoned")
                .remove(&id);
            ControlResponse::failure(id, "frontend_timeout", "Micah UI did not respond in time")
        }
        Err(mpsc::RecvTimeoutError::Disconnected) => ControlResponse::failure(
            id,
            "frontend_unavailable",
            "Micah UI response channel closed",
        ),
    }
}
...
fn prepare_cli_launcher(descriptor: &Path, cli_path: &Path) -> Result<PathBuf, String> {
    ...
    let launcher = bin_dir.join(if cfg!(windows) { "micah.exe" } else { "micah" });
