...
fn run(args: Vec<OsString>) -> Result<(), CliError> {
    ...
    match config.action {
        ...
        Action::Version => {
            println!("micah {}", env!("CARGO_PKG_VERSION"));
            Ok(())
        }
        Action::Request { method, params } => {
            let endpoint = load_endpoint()?;
            let caller = env::var("MICAH_PANE_ID")
                .ok()
            ...
            if !response.ok {
                let error = response.error.unwrap_or_else(|| {
                    micah_control_protocol::ControlError::new(
                        "request_failed",
                        "Micah rejected the request",
                    )
                });
...
fn parse_open(args: Vec<OsString>) -> Result<Action, CliError> {
    ...
    let path = canonical.into_os_string().into_string().map_err(|_| {
        CliError::new(
            "non_utf8_path",
            "Micah cannot open a path that is not valid UTF-8",
            EXIT_USAGE,
        )
    })?;
...
fn load_endpoint() -> Result<ControlDescriptor, CliError> {
    let env_address = env::var("MICAH_CONTROL_ADDR").ok();
    let env_token = env::var("MICAH_CONTROL_TOKEN").ok();
    let (descriptor, require_live_process) = match (env_address, env_token) {
        ...
        (None, None) => {
            let path = dirs::cache_dir()
                .map(|dir| dir.join("micah").join("control.json"))
                .ok_or_else(|| {
                    CliError::new(
                        "app_unavailable",
            ...
            let bytes = std::fs::read(&path).map_err(|_| {
                CliError::new(
                    "app_unavailable",
                    "Micah is not running; start the app and try again",
                    EXIT_UNAVAILABLE,
                )
            })?;
            let descriptor = serde_json::from_slice(&bytes).map_err(|error| {
                CliError::new(
                    "invalid_descriptor",
                    format!("invalid Micah control descriptor: {error}"),
                    EXIT_PROTOCOL,
                )
            })?;
            (descriptor, true)
        ...
        _ => {
            return Err(CliError::new(
                "invalid_environment",
                "MICAH_CONTROL_ADDR and MICAH_CONTROL_TOKEN must be set together",
                EXIT_PROTOCOL,
            ));
        }
    };
...
fn validate_endpoint(
    ...
    if descriptor.protocol != PROTOCOL_VERSION {
        return Err(CliError::new(
            ...
            format!(
                "Micah uses control protocol {}, but this CLI supports {PROTOCOL_VERSION}",
                descriptor.protocol
            ),
            EXIT_PROTOCOL,
        ));
            ...
            .all(|byte| byte.is_ascii_hexdigit())
    {
        return Err(CliError::new(
            "invalid_endpoint",
            "Micah control token is invalid",
            EXIT_PROTOCOL,
        ));
    }
    ...
    if require_live_process && !process_is_alive(descriptor.pid) {
        return Err(CliError::new(
            "invalid_endpoint",
            "Micah control process is not running",
            EXIT_PROTOCOL,
        ));
    }
    Ok(descriptor)
...
fn send_request(address: &str, request: &ControlRequest) -> Result<ControlResponse, CliError> {
    ...
    let mut stream = TcpStream::connect_timeout(&address, CONNECT_TIMEOUT).map_err(|error| {
        CliError::new(
            "app_unavailable",
            format!("could not connect to Micah: {error}"),
            EXIT_UNAVAILABLE,
        )
    })?;
...
fn parse_loopback_address(address: &str) -> Result<SocketAddr, CliError> {
    let address: SocketAddr = address.parse().map_err(|error| {
        CliError::new(
            "invalid_endpoint",
            format!("invalid Micah control address: {error}"),
            EXIT_PROTOCOL,
        )
    })?;
    if !address.ip().is_loopback() {
        return Err(CliError::new(
            "invalid_endpoint",
            "Micah control address must be loopback-only",
            EXIT_PROTOCOL,
        ));
    }
    Ok(address)
...
fn read_response(
    ...
    if bytes.len() > MAX_MESSAGE_BYTES {
        return Err(CliError::new(
            "message_too_large",
            "Micah response exceeded the protocol limit",
            EXIT_PROTOCOL,
        ));
    }
    if bytes.last() != Some(&b'\n') {
        return Err(CliError::new(
            "invalid_response",
            "Micah returned an incomplete response",
            EXIT_PROTOCOL,
        ));
    }
    let response: ControlResponse = serde_json::from_slice(&bytes).map_err(|error| {
        CliError::new(
            "invalid_response",
            format!("Micah returned invalid JSON: {error}"),
            EXIT_PROTOCOL,
        )
    })?;
    let matched_id =
    ...
    if response.protocol != PROTOCOL_VERSION || !matched_id {
        return Err(CliError::new(
            "invalid_response",
            "Micah returned a mismatched protocol version or request id",
            EXIT_PROTOCOL,
        ));
    }
    Ok(response)
    ...
    match method {
        METHOD_PING => {
            let version = result
                ...
                .unwrap_or("unknown");
            println!("Micah {version} is running");
        }
        METHOD_CAPABILITIES => {
        ...
        METHOD_OPEN => {
            ...
            if let Some(line) = line {
                println!("Opened {path}:{line} in Micah");
            } else {
                println!("Opened {path} in Micah");
            }
        }
        _ => println!("{result}"),
    }
...
fn print_help() {
    println!(
        "Micah command line interface\n\n\
Usage:\n  micah <file> [--line <n>] [--no-focus] [--json]\n  micah open <file> [--line <n>] [--no-focus] [--json]\n  micah ping [--json]\n  micah capabilities [--json]\n  micah identify [--json]\n  micah --version\n\n\
The app must be running. Commands launched in a Micah pane target that pane's space."
    );
}
